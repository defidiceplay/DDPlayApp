...
// CLAP value
    uint256 clapValue = 5 * 10 ** 6; // 10 ** 8 = 1$ // 0.05$
...
// Pool Address
    address public addressForPool = 0xD633879d1F4F86444E66f3146407a9DC83c116e1;
    function set_addressForPool(address new_addressForPool) public Owner { addressForPool = new_addressForPool; }
    function get_addressForPool() public view returns (address) { return addressForPool; }
...
// Earn
    ...
    uint256[10] rewards = [5, 100, 150, 225, 325, 450, 600, 775, 975, 1000];
    function set_rewards(uint256[10] memory new_rewards) public Owner { rewards = new_rewards; }
...
// Get
    ...
    function getReward(uint256 result) private view returns (uint256) {
        if (result > 9) return rewards[9];
...
// Set
    ...
    function boardPlay(address user, uint256 result) external NFT {
        ...
        uint256 percVictory = getReward(result);
...
// Airdrop testnet (ONLY BSC TESTNET!)
